...
func (h *AppHeader) buildLeftSection(bg tcell.Color) *tview.TextView {
	...
	// Modern Wooak branding
	stylizedName := "🚀 [#39FF39::b]WOOAK[-] [#AAAAAA]SSH Manager[-]"
...
func (h *AppHeader) buildCenterSection(bg tcell.Color) *tview.TextView {
	...
	// Build modern tag-like chips for version, commit, and build time
	versionTag := makeTag(h.version, "#46FF46") // bright green
	commitTag := ""
	if commit != "" {
		commitTag = makeTag(commit, "#39BFFF") // bright blue
	}
...
func (h *AppHeader) buildRightSection(bg tcell.Color) *tview.TextView {
	...
	right.SetText("[#39BFFF::u]🔗 " + h.repoURL + "[-]  [#AAAAAA]• " + currentTime + "[-]")
	return right
}
