...
// Tab management functions moved to server_form_tabs.go
...
// Autocomplete functions moved to server_form_autocomplete.go
...
func (sf *ServerForm) serversDiffer(a, b domain.Server) bool {
	...
	// Fields to skip during comparison (wooak metadata fields)
...
func (sf *ServerForm) dataToServer(data ServerFormData) domain.Server {
	...
	server.ProxyCommand = data.ProxyCommand
	server.RemoteCommand = data.RemoteCommand
	server.RequestTTY = data.RequestTTY
	server.SessionType = data.SessionType
	server.ConnectTimeout = data.ConnectTimeout
	server.ConnectionAttempts = data.ConnectionAttempts
	server.BindAddress = data.BindAddress
	server.BindInterface = data.BindInterface
	server.AddressFamily = data.AddressFamily
	server.ExitOnForwardFailure = data.ExitOnForwardFailure
	server.LocalForward = splitComma(data.LocalForward)
	server.RemoteForward = splitComma(data.RemoteForward)
	server.DynamicForward = splitComma(data.DynamicForward)
	server.ClearAllForwardings = data.ClearAllForwardings
	// Public key
	server.PubkeyAuthentication = data.PubkeyAuthentication
	server.IdentitiesOnly = data.IdentitiesOnly
	// SSH Agent
	server.AddKeysToAgent = data.AddKeysToAgent
	server.IdentityAgent = data.IdentityAgent
	// Password & Interactive
	server.PasswordAuthentication = data.PasswordAuthentication
	server.KbdInteractiveAuthentication = data.KbdInteractiveAuthentication
	server.NumberOfPasswordPrompts = data.NumberOfPasswordPrompts
	// Advanced
	server.PreferredAuthentications = data.PreferredAuthentications
	server.ForwardAgent = data.ForwardAgent
	server.ForwardX11 = data.ForwardX11
	server.ForwardX11Trusted = data.ForwardX11Trusted
	server.ControlMaster = data.ControlMaster
	server.ControlPath = data.ControlPath
	server.ControlPersist = data.ControlPersist
	server.ServerAliveInterval = data.ServerAliveInterval
	server.ServerAliveCountMax = data.ServerAliveCountMax
	server.Compression = data.Compression
	server.TCPKeepAlive = data.TCPKeepAlive
	server.BatchMode = data.BatchMode
	server.StrictHostKeyChecking = data.StrictHostKeyChecking
	server.UserKnownHostsFile = data.UserKnownHostsFile
	server.HostKeyAlgorithms = data.HostKeyAlgorithms
	server.PubkeyAcceptedAlgorithms = data.PubkeyAcceptedAlgorithms
	server.HostbasedAcceptedAlgorithms = data.HostbasedAcceptedAlgorithms
	server.MACs = data.MACs
	server.Ciphers = data.Ciphers
	server.KexAlgorithms = data.KexAlgorithms
	server.VerifyHostKeyDNS = data.VerifyHostKeyDNS
	server.UpdateHostKeys = data.UpdateHostKeys
	server.HashKnownHosts = data.HashKnownHosts
	server.VisualHostKey = data.VisualHostKey
	server.LocalCommand = data.LocalCommand
	server.PermitLocalCommand = data.PermitLocalCommand
	server.EscapeChar = data.EscapeChar
	server.SendEnv = splitComma(data.SendEnv)
	server.SetEnv = splitComma(data.SetEnv)
	server.LogLevel = data.LogLevel
